...
ML4FILES:=src/timing_plugin.ml4
...
install-natdynlink:
	cd src; for i in $(CMXSFILES1); do \
	 install -d `dirname $(DSTROOT)$(COQLIBINSTALL)/Timing/$$i`; \
	 install -m 0644 $$i $(DSTROOT)$(COQLIBINSTALL)/Timing/$$i; \
	done
...
install-doc:
	install -d $(DSTROOT)$(COQDOCINSTALL)/Timing/html
	for i in html/*; do \
	 install -m 0644 $$i $(DSTROOT)$(COQDOCINSTALL)/Timing/$$i;\
	done
